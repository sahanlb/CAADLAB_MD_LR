        ...
        for(int hp = 0; hp<cell_p_count; hp++){
          for(int n=0; n<27; n++){ // go through each of the neighbor cells
            ...
            for(int np=0; np<nb_p_count; np++){ // go through each of the neighbor particles
              ...
              float h_x_neg, h_x_pos, n_x_neg, n_x_pos;
              float h_y_neg, h_y_pos, n_y_neg, n_y_pos;
              float h_z_neg, h_z_pos, n_z_neg, n_z_pos;
              
              h_x_neg = floor(h_x/GRID_DIST_X)*GRID_DIST_X - GRID_DIST_X;
              h_x_pos = floor(h_x/GRID_DIST_X)*GRID_DIST_X + 2*GRID_DIST_X;

              n_x_neg = floor(n_x/GRID_DIST_X)*GRID_DIST_X - GRID_DIST_X;
              n_x_pos = floor(n_x/GRID_DIST_X)*GRID_DIST_X + 2*GRID_DIST_X;

              h_y_neg = floor(h_y/GRID_DIST_Y)*GRID_DIST_Y - GRID_DIST_Y;
              h_y_pos = floor(h_y/GRID_DIST_Y)*GRID_DIST_Y + 2*GRID_DIST_Y;

              n_y_neg = floor(n_y/GRID_DIST_Y)*GRID_DIST_Y - GRID_DIST_Y;
              n_y_pos = floor(n_y/GRID_DIST_Y)*GRID_DIST_Y + 2*GRID_DIST_Y;

              h_z_neg = floor(h_z/GRID_DIST_Z)*GRID_DIST_Z - GRID_DIST_Z;
              h_z_pos = floor(h_z/GRID_DIST_Z)*GRID_DIST_Z + 2*GRID_DIST_Z;

              n_z_neg = floor(n_z/GRID_DIST_Z)*GRID_DIST_Z - GRID_DIST_Z;
              n_z_pos = floor(n_z/GRID_DIST_Z)*GRID_DIST_Z + 2*GRID_DIST_Z;


              if(
                (((h_x > n_x) && (h_x_neg <= n_x_pos)) || ((h_x <= n_x) && (h_x_pos >= n_x_neg))) &&
                (((h_y > n_y) && (h_y_neg <= n_y_pos)) || ((h_y <= n_y) && (h_y_pos >= n_y_neg))) &&
                (((h_z > n_z) && (h_z_neg <= n_z_pos)) || ((h_z <= n_z) && (h_z_pos >= n_z_neg)))
                ){
                  ...
                  if(cell_index == 0 && nb_cell_id == 1 && hp == 0){
                    cout << "Overlapping sub-grids. HP: " << h_x << ", " << h_y << ", " << h_z << "\tNP: " << n_x << ", " << n_y << ", " << n_z << endl;
                  }
                  /** debug print **/
